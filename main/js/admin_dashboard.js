...
document.querySelectorAll(".sidebar-menu a").forEach((link) => {
  link.addEventListener("click", function (e) {
    ...
    if (section) {
      // Only change active section if it's different from current
      const currentActiveSection = document.querySelector(
        ".form-section.active"
      );
      if (!currentActiveSection || currentActiveSection.id !== section) {
        document
...
window.showAddForm = function (formId) {
  document
    .querySelectorAll(".form-section .card")
...
window.backToList = function (section) {
  ...
  // Only hide cards in the current section, not all sections
  const currentSectionCards = document.querySelectorAll(`#${section} .card`);
  if (currentSectionCards.length === 0) {
    console.warn(`No cards found in section: ${section}`);
  }
  currentSectionCards.forEach((card) => {
    card.style.display = "none";
  });

  // Show the correct list card
  const listId = `${section.slice(0, -1)}-list`; // Remove 's' and add '-list'
  ...
  if (listElement) {
    ...
    Swal.fire({
      icon: "error",
      title: "Error",
      text: `List element not found: ${listId}.`,
    });
  }
};
  ...
  fetch(`api/get_student_details.php?id=${id}`)
    ...
    .then((data) => {
      document.getElementById("studentDetailsContent").innerHTML = `
            ...
            `;
      document
        .querySelectorAll("#students .card")
        .forEach((card) => (card.style.display = "none"));
  ...
  fetch(`api/get_teacher_details.php?id=${id}`)
    ...
    .then((data) => {
      document.getElementById("teacherDetailsContent").innerHTML = `
            ...
            `;
      document
        .querySelectorAll("#teachers .card")
        .forEach((card) => (card.style.display = "none"));
...
document
  .getElementById("updateParentForm")
  .addEventListener("submit", function (e) {
    e.preventDefault();
    const submitBtn = this.querySelector('button[type="submit"]');
    const originalText = submitBtn.innerHTML;
